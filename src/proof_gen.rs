...
use crate::utils::{
    core_utilities::{
        calculate_domain, calculate_random_scalars, hash_to_scalar, mocked_calculate_random_scalars,
    },
    interface_utilities::{create_generators, msg_to_scalars, HashToG1},
    utilities_helper::FromOkm,
};
use crate::{constants::Constants, key_gen::PublicKey, sign::Signature};
...
pub enum ProofGenError {
    ...
    #[error(
        "Invalid disclosed index: The disclosed index cannot be greater than the messages length."
    )]
...
pub fn proof_gen<E, F, H, C>(
    pk: PublicKey<E>,
    signature: Signature<E, F>,
    header: &[u8],
    ph: &[u8],
    messages: &[&[u8]],
    disclosed_indexes: &[usize],
) -> Result<Proof<E, F>, ProofGenError>
where
    E: Pairing,
...
pub(crate) fn core_proof_gen<E, F, C>(
    pk: PublicKey<E>,
    signature: Signature<E, F>,
    header: &[u8],
    generators: &[E::G1],
...
pub(crate) fn proof_challenge_calculate<E, F>(
    init_res: &InitProof<E, F>,
    disclosed_messages: &[F],
    disclosed_indexes: &[usize],
    ph: &[u8],
    api_id: &[u8],
) -> Result<Challenge<F>, ProofGenError>
where
    E: Pairing,
    F: Field + FromOkm<48, F>,
{
...
pub(crate) fn proof_finalize<E, F>(
    init_res: &InitProof<E, F>,
    challenge: &Challenge<F>,
    e_value: F,
    random_scalars: &[F],
    undisclosed_messages: &[F],
) -> Result<Proof<E, F>, ProofGenError>
where
    E: Pairing,
    F: Field,
