...
mod tests {
    ...
    fn generate_key() -> (SecretKey<Fr>, PublicKey<Bn254>) {
        ...
        let sk =
            SecretKey::<Fr>::key_gen::<Bn254>(&mut key_material, &[], key_dst.as_slice()).unwrap();
    ...
    fn test_proof_verify(count: usize, disclosed_indexes: Vec<usize>, header: &[u8]) {
        ...
        let signature = sk
            .sign::<Bn254, Bn254Const, HashToG1Bn254>(&msg_slices, header)
            .unwrap();
        assert!(pk
            .verify::<Fr, HashToG1Bn254, Bn254Const>(signature, header, &msg_slices)
            .unwrap());

        let disclosed_msgs: Vec<&[u8]> = disclosed_indexes.iter().map(|i| msg_slices[*i]).collect();

        let proof = proof_gen::<Bn254, Fr, HashToG1Bn254, Bn254Const>(
            pk.clone(),
            signature,
            header,
            &[],
        ...
        assert!(proof_verify::<Bn254, Fr, HashToG1Bn254, Bn254Const>(
            pk,
            proof.unwrap(),
            header,
            &[],
    ...
    fn test_invalid_proof() {
        ...
        let msg_slices: Vec<&[u8]> = messages.iter().map(|v| v.as_slice()).collect();
        let signature = sk
            .sign::<Bn254, Bn254Const, HashToG1Bn254>(&msg_slices, b"")
            .unwrap();
        let proof = proof_gen::<Bn254, Fr, HashToG1Bn254, Bn254Const>(
            pk.clone(),
            signature,
