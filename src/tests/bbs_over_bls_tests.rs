...
mod tests {
    use ark_bls12_381::{Bls12_381, Fr, G1Affine as G1};
    use rand::Rng;
    ...
    fn generate_key() -> (SecretKey<Fr>, PublicKey<Bls12_381>) {
        ...
        let sk = SecretKey::<Fr>::key_gen::<Bls12_381>(&mut key_material, &[], key_dst.as_slice())
            .unwrap();
    ...
    fn test_proof_verify(count: usize, disclosed_indexes: Vec<usize>, header: &[u8]) {
        ...
        let signature = sk
            .sign::<Bls12_381, Bls12381Const, HashToG1Bls12381>(&msg_slices, header)
            .unwrap();
        assert!(pk
            .verify::<Fr, HashToG1Bls12381, Bls12381Const>(signature, header, &msg_slices)
            .unwrap());
        ...
        assert!(
            proof_verify::<Bls12_381, Fr, HashToG1Bls12381, Bls12381Const>(
                pk,
                proof.unwrap(),
                header,
                &[],
                disclosed_msgs.as_slice(),
                disclosed_indexes.as_slice()
            )
            .unwrap()
        );
    ...
    fn test_invalid_proof() {
        ...
        let msg_slices: Vec<&[u8]> = messages.iter().map(|v| v.as_slice()).collect();
        let signature = sk
            .sign::<Bls12_381, Bls12381Const, HashToG1Bls12381>(&msg_slices, b"")
            .unwrap();
        let proof = proof_gen::<Bls12_381, Fr, HashToG1Bls12381, Bls12381Const>(
            pk.clone(),
            signature,
        ...
        assert!(
            proof_verify::<Bls12_381, Fr, HashToG1Bls12381, Bls12381Const>(
                ...
                b"",
                &[],
                &[&msg_slices[0], &msg_slices[1], &msg_slices[5]],
                &[0, 1, 5]
            )
            .unwrap()
        );
