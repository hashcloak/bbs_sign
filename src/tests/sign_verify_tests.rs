...
mod tests {
    ...
    fn test_invalid_signature() {
        ...
        assert!(!pk
            .verify::<Fr, HashToG1Bn254, Bn254Const>(forged_signature, header, &msg_slices)
            .unwrap());

        // forged header
        let forged_header = b"abc";
        assert!(!pk
            .verify::<Fr, HashToG1Bn254, Bn254Const>(signature, forged_header, &msg_slices)
            .unwrap());
        ...
        assert!(!pk
            .verify::<Fr, HashToG1Bn254, Bn254Const>(signature, header, &forged_messages)
            .unwrap());
    }
}
